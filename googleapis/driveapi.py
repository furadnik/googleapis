...
class File:
    ...
    def push(self, path, remove_nonexisting=True, subfolders=None, hidden=True):
        if not self.is_dir:raise Exception("You can push or pull only from/to a dir")
        path = pathlib.Path(path)
        local, drive = os.listdir(path), self.get_folder_content(folders=True)

        if not subfolders is None:local = [x for x in local if x in subfolders]
        if not hidden:local = [x for x in local if not x.startswith(".")]
        for x in local:
            file_path = path / x
            corresponding_drive = [y for y in drive if y.name == x and y.is_dir == file_path.is_dir()]
            if file_path.is_dir():
                if len(corresponding_drive) == 1:corresponding_drive[0].push(file_path, remove_nonexisting, hidden=hidden)
                elif remove_nonexisting or len(corresponding_drive) == 0:
                    for d in corresponding_drive:d.remove()
                    self.upload(file_path, hidden=hidden)

            elif len(corresponding_drive) > 1:pass
            elif len(corresponding_drive) == 0:self.upload(file_path)
            else:
        ...
        if remove_nonexisting and subfolders is None:
            for x in drive:
                if not x.name in local:x.remove()

    def pull(self, path, remove_nonexisting=True, subfolders=None, hidden=True):
        if not self.is_dir:raise Exception("You can push or pull only from/to a dir")
        path = pathlib.Path(path)
        if not path.exists():os.makedirs(path)
        if not path.is_dir():raise Exception("Local path isn't dir")
        local, drive = os.listdir(path), self.get_folder_content(folders=True)
        checked = []

        if not subfolders is None:drive = [x for x in drive if x.name in subfolders]
        if not hidden: drive = [x for x in drive if not x.name.startswith('.')]
        for x in drive:
            checked.append(x.name)
            if x.is_dir and x.name in local:x.pull(path / x.name, remove_nonexisting)
            elif (x.is_gdoc):x.download(path)
        ...
        if remove_nonexisting:
            for x in local:
                if not x in checked:
                    if (path/x).is_dir():shutil.rmtree(path / x)
                    else:os.remove(path / x)
    ...
    def set_content(self, content, mt='application/octet-stream'):
        if isinstance(content, str):
            ...
            mt = "text/plain"
        if len(content) == 0:media = None
        else:media = MediaInMemoryUpload(content, resumable=True, mimetype=mt)
    ...
    def upload(self, path, name=None, exclude_partial=False, hidden=True):
        ...
        if os.path.exists(path) and os.path.isfile(path):
            if exclude_partial and (name.endswith(".crdownload") or name.endswith(".part") or name + ".part" in os.listdir(path.parent)):return
            if hidden and (name.startswith(".")):return
            ...
            file_metadata = {'name': name,
                            'parents': [self.id]}
            if empty:media = None
            else:media = MediaFileUpload(path, resumable=True)
    ...
    def download(self, folder_path):
        ...
        if self.is_dir:
            ...
            if self.mime_type == "application/vnd.google-apps.script":return False
            if self.is_gdoc:request = drive_service.files().export_media(fileId=self.id, mimeType='application/pdf')
            else:request = drive_service.files().get_media(fileId=self.id)

            if self.size == 0:
                with open(path / self.name, "w") as f:f.write("")
                return True
    ...
    def get_folder_content(self, folders=False):
        if not folders:mt = " and mimeType != 'application/vnd.google-apps.folder'"
        else:mt = ""
    ...
    def get_or_create(self, name, folder=False):
        ...
        if len(files) and (files[0].is_dir or folder == False): return files[0]
        if folder:return self.create_folder(name)
        return self.create_file(name)

    def move_to(self, folder, from_id=None):
        if from_id is None:from_id = self.parent.id
...
def get_file(id):
    try:return File(drive_service.files().get(fileId=id, fields=FIELDS).execute())
    except:return None
...
def _md5_from_local_file(path):
    ...
    with open(path, "rb") as f:
        ...
        while piece:=f.read(block_size):r.update(piece)

    return r.hexdigest()



drive_service = googleapi.get_service('drive')
