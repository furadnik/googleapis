...
def list_events(calendar="primary", date=None, max_results=10, force_day=True):
    ...
    date = todt(date)
    date = todt(date.date())
    if force_day:timeMax = get_google_from_dt(date + datetime.timedelta(days=1)) + "Z"
    else:timeMax = None
...
def todt(dt):
    if not isinstance(dt, datetime.datetime):
        if isinstance(dt, datetime.date):dt = datetime.datetime.combine(dt, datetime.datetime.min.time())
        elif isinstance(dt, datetime.time):dt = datetime.datetime.combine(datetime.date.today(), dt)
    return dt
    ...
    if isinstance(dt, dict):
        if "timeZone" in dt.keys():
            if dt["timeZone"] != LOCAL_TIMEZONE:
                dif = get_utc_offset()
        if "dateTime" in dt.keys():dt = dt["dateTime"]
        else:dt = dt["date"]; form = "%Y-%m-%d"
    return datetime.datetime.strptime(dt, form).replace(tzinfo=None) + dif

def create_event(name, start, end=None, description=None, location=None, color=None):
    """
    COLORS
    1 blue
    2 green
    ...
    if color:event["colorId"] = str(color)
...
def import_from_ics(file_name):
    ...
    if file_name.exists():
        ...
        for line in content:
            ...
            if line.startswith("DTSTART:"):event["start"] = datetime.datetime.strptime(line.split(":", 1)[1], "%Y%m%dT%H%M%S%z")
            if line.startswith("DTEND:"):event["end"] =   datetime.datetime.strptime(line.split(":", 1)[1], "%Y%m%dT%H%M%S%z")
            if line.startswith("SUMMARY:"):event["sum"] =  line.split(":", 1)[1]
            if line.startswith("DESCRIPTION:"):event["des"] = line.split(":", 1)[1].replace("\\n", "\n")
            if line == "END:VEVENT": create_event(event["sum"], event["start"], event["end"], event["des"])
            print(event);
